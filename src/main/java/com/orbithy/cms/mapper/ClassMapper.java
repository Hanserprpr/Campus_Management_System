...
public interface ClassMapper extends BaseMapper<Classes> {
    ...
    @Select("SELECT id, class_num, name, point, term, status, regular_ratio, final_ratio, type FROM classes WHERE term = #{term} LIMIT #{offset}, #{size}")
    ...
    @Select("SELECT id, class_num, name, point, term, status, regular_ratio, final_ratio, type FROM classes LIMIT #{offset}, #{size}")
