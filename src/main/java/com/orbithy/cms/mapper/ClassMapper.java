...
public interface ClassMapper extends BaseMapper<Classes> {
    ...
    @Select("SELECT COUNT(*) FROM classes WHERE status IN (1, 3)")
    int countAllCourses();
    ...
    @Select("SELECT * FROM classes WHERE id = #{courseId} AND term = #{term}")
    Classes getCourseByTermId(Integer courseId, String term);
}